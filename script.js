...
function openWidget(evt, widgetName) {
  ...
  if (widgetName == "windfinder") {
    ...
    s.src =
      "https://www.windfinder.com/widget/forecast/js/barra?unit_wave=m&unit_rain=mm&unit_temperature=c&unit_wind=kmh&unit_pressure=hPa&days=4&show_day=0";
    widgetDiv.appendChild(s);
  } else if (widgetName == "windy") {
    document.getElementById("windy-widget").innerHTML =
      '<iframe src="https://embed.windy.com/embed2.html?lat=40.643&lon=-8.745&detailLat=40.643&detailLon=-8.745&width=800&height=600&zoom=10&level=surface&overlay=waves&product=ecmwf&menu=&message=true&marker=&calendar=12&pressure=&type=map&location=coordinates&detail=&metricWind=default&metricTemp=default&radarRange=-1&key=WkB9eYcd1J6eTaWz7f8M1Hknm2jHEnOH" frameborder="0" style="width: 100%; height: 502px;"></iframe>';
  }
}
...
map.on("load", () => {
  ...
  function createPopupHTMLPI(
    ...
    if (tabela === "ondas") {
      extraHTML = `
        <p><button class="btnPraias" id="saber-mais" data-extra-info='${JSON.stringify(
          extraInfo
        )}'>Ver Informações
  ...
  function openModalWithExtraInfo(extraInfo) {
    ...
    var extraHTML = `
      ...
      <p><b>Velocidade do Vento:</b> ${
        extraInfo.velocidadevento || "Desconhecido"
      }</p>
      <p><b>Altura da Onda:</b> ${extraInfo.alturaonda || "Desconhecido"}</p>
      <p><b>Direção do Vento:</b> ${
        extraInfo.direcaovento || "Desconhecido"
      }</p>
      <p><b>Altura do 'swell':</b> ${
        extraInfo.swellaltura || "Desconhecido"
      }</p>
      <p><b>Período do 'swell':</b> ${
        extraInfo.swellperiodo || "Desconhecido"
      }</p>
      <p><b>Direção do 'swell':</b> ${
        extraInfo.swelldirecao || "Desconhecido"
      }</p>
  ...
  function loadStream(videoElementId, streamUrl) {
    ...
    var DEBUG_MODE = false; // Mude para true se quiser ver os erros no console.
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, function () {
        video
          .play()
          .catch(() => logError("Autoplay bloqueado pelo navegador."));
      });

      hls.on(Hls.Events.ERROR, function (event, data) {
        if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
          logError(`Erro ao carregar o stream: ${streamUrl}`);
        }
      });
  ...
  function addLayers() {
    ...
    tabelas.forEach((tabela) => {
          ...
          map.loadImage(getLayerImage(tabela), function (error, image) {
            ...
            map.on("mouseenter", tabela + "_within", function (e) {
              ...
              const nome = e.features[0].properties.nome
                ? e.features[0].properties.nome
                : "Desconhecido";
              const imgurl = e.features[0].properties.imgurl
                ? e.features[0].properties.imgurl
                : "Imagem desconhecida";
              ...
              fetch(
                ...
                .then((data) => {
                  ...
                  const rua = addressParts.road
                    ? addressParts.road
                    : "Desconhecido";
                  const codigoPostal = addressParts.postcode
                    ? addressParts.postcode
                    : "Desconhecido";
                  const cidade = addressParts.city
                    ? addressParts.city
                    : "Desconhecido";
                  const addressHTML = `
                  ...
                  document
                    .getElementById("add_PI_to_Route")
              ...
              fetch(
                ...
                .then((text) => {
                  try {
                    ...
                    if (data.error) return; // Se houver erro na resposta, simplesmente sai da função

                    const addressParts = data.address;
                    const rua = addressParts.road || "Desconhecido";
                    const codigoPostal =
                      addressParts.postcode || "Desconhecido";
                    const cidade = addressParts.city || "Desconhecido";
      ...
      `;
                    ...
                    // Ignoramos o erro completamente, sem exibir nada no console
                  }
                })
                .catch(() => {
                  // Também ignoramos erros de rede
                });
            });
  ...
  percursos.forEach(function (percurso) {
    fetch(
      `https://gis4cloud.com/grupo4_ptas2024/percursos.php?tabela=${percurso}`
    )
      .then((response) => response.json())
  ...
  percursos_nauticos.forEach(function (percurso_nautico) {
    fetch(
      `https://gis4cloud.com/grupo4_ptas2024/percursos_nauticos.php?tabela=${percurso_nautico}`
    )
      .then((response) => response.json())
      .then((data) => {
...
function selectPeriod(periodType) {
  const isSelected = document
    .querySelector(`.periods .option[data-value="${periodType}"]`)
...
function calculateIsochrone() {
  ...
  var routingProfile = selectedVehicle
    ? selectedVehicle.getAttribute("data-value")
    : null;

  var selectedMetric = document.querySelector(".metrica .option.selected");
  var contourType = selectedMetric
    ? selectedMetric.getAttribute("data-value")
    : null;
  ...
  fetch(apiUrl)
    ...
    .then((data) => {
      ...
      const periodType = document
        .querySelector(".periods .option.selected")
...
document
  .getElementById("addPointIntermedio")
  .addEventListener("click", function () {
    ...
    $(`#${inputId}`).autocomplete({
      source: function (request, response) {
        ...
        axios
          ...
          .then((res) => {
            ...
            response(
              featuresWithinBounds.map((feature) => ({
      ...
      select: function (event, ui) {
        ...
        if (inputId === "addressInputA") {
          ...
          if (markerIntermedio) markerIntermedio.remove();
          markerIntermedio = new mapboxgl.Marker({
            color: markerColor,
            draggable: true,
          })
            .setLngLat(coords)
  ...
  fetch(apiUrl)
    ...
    .then((data) => {
      ...
      map.on("mousemove", "route", function (e) {
        ...
        var popupHTML = createPopupHTMLRoute(
      ...
      if (selectedCategory) {
        fetch(
          `https://gis4cloud.com/grupo4_ptas2024/bd.php?tabela=${selectedCategory}`
        )
          .then((response) => {
            if (!response.ok) {
              throw new Error(
                `Erro na requisição de pontos de interesse: ${response.status}`
              );
            }
            return response.json();
          })
          .then((pointsOfInterest) => {
            var nearestPointOfInterest = pointsOfInterest.features.reduce(
              (nearestPoint, currentPoint) => {
                var distanceToRoute = turf.pointToLineDistance(
                  currentPoint,
                  route
                );
                if (
                  !nearestPoint ||
                  distanceToRoute < nearestPoint.distanceToRoute
                ) {
                  return {
                    point: currentPoint,
              ...
              fetch(apiUrl)
                ...
                .then((data) => {
                  if (
                    ...
                    console.warn(
                      "Não há direções disponíveis para esta rota recalculada."
                    );
                    return;
                  }
                  ...
                  var distance =
                    turf.length(newRoute, { units: "kilometers" }) * 1000;
                  ...
                  if (markerPI_maisProximo) {
                    ...
                    markerPI_maisProximo = new mapboxgl.Marker({
                      ...
                      .setLngLat(
                        nearestPointOfInterest.point.geometry.coordinates
                      )
                      .addTo(map);
                    ...
                    popupMaisProximo = new mapboxgl.Popup({ offset: 25 })
                      ...
                      .setText(
                        "Ponto da categoria de ponto de interesse mais próximo"
                      )
                      .addTo(map);
                  ...
                  map.on("mousemove", "route", function (e) {
                    var tripDurationInMinutes = Math.round(
                      data.routes[0].duration / 60
                    );
                    var tripDistanceInMeters = Math.round(
                      data.routes[0].distance
                    );
                    var directions = data.routes[0].legs[0].steps;

                    var formattedTripDuration = formatTripDuration(
                      tripDurationInMinutes
                    );
                    var formattedTripDistance =
                      formatTripDistance(tripDistanceInMeters);
