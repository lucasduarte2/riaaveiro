 ...
 */
  ...
  function addClusterMap() {
    tabelas.forEach((tabela) => {
      ...
      fetch(`bd.php?tabela=${tabela}`)
        ...
        .then((data) => {
          const sourceId = `cluster-data-${tabela}`;
          const clusterLayerId = `cluster-layer-${tabela}`;
          const clusterCountLayerId = `cluster-count-layer-${tabela}`;
          const unclusteredPointLayerId = `unclustered-point-layer-${tabela}`;
  
          if (!map.getSource(sourceId)) {
            // Adiciona a fonte 'cluster-data' ao mapa
            map.addSource(sourceId, {
              type: 'geojson',
              data: data,
              cluster: true,
              clusterMaxZoom: 14, // Zoom máximo para agrupar pontos
              clusterRadius: 50 // Raio de cada cluster ao agrupar pontos (padrão é 50)
            });
  
            // Adiciona a camada de clusters ao mapa
            map.addLayer({
              id: clusterLayerId,
              type: 'circle',
              source: sourceId,
              ...
              paint: {
                'circle-color': '#FF0000', // Vermelho constante para todos os clusters
                'circle-radius': [
                  'step',
                  ['get', 'point_count'],
                  20, // Raio do cluster para pequenos clusters
                  100,
                  30, // Raio do cluster para clusters médios
                  750,
                  40 // Raio do cluster para grandes clusters
                ],
                'circle-stroke-width': 1,
                'circle-stroke-color': '#FF0000' // Contorno branco para visibilidade
              }
            });
   
            // Adiciona a camada de contagem de clusters ao mapa
            map.addLayer({
              ...
              paint: {
                'text-color': '#FFFFFF' // Define a cor do texto como vermelho
              }
            });
  
            // Adiciona a camada de pontos não agrupados ao mapa
            map.addLayer({
              ...
              filter: ['!', ['has', 'point_count']],
              paint: {
                'circle-color': '#FF0000', // Vermelho para pontos não agrupados
                'circle-radius': 6,
                'circle-stroke-width': 1,
                'circle-stroke-color': '#FF0000' // Contorno branco para visibilidade
              }
  ...
  // Função para remover o mapa de clusters
  function removeClusterMap() {
    tabelas.forEach((tabela) => {
      ...
      // Remove a fonte de dados do cluster
      if (map.getSource(sourceId)) {
        map.removeSource(sourceId);
      }
    });
  ...
  // Adiciona um event listener para o checkbox do cluster
  document.getElementById("clusterMap").addEventListener("change", function () {
    if (this.checked) {
      addClusterMap();
    } else {
      removeClusterMap();
